...
Add Verso at the workspace root and expose it through a script:
...
Use `pnpm exec verso ...` for direct commands, or `pnpm release -- ...` through the script. The npm
package resolves the platform package, repairs the executable bit on macOS and Linux when needed,
