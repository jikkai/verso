...
Starting another mutating release or bump while a transaction is active first prompts to resume or
abort the recorded transaction. The requested new release is not started in the same invocation.
...
After manually inspecting or recovering an exceptional state, `abort --force` discards the active
journal without parsing its plan or changing local files, commits, tags, or remote refs. It is an
unlock operation, not an automatic rollback.
