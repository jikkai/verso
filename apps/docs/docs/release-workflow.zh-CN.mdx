...
<Callout variant="danger" title="Push 是恢复边界">
  Push 一旦开始，由于远端结果可能未知，abort 就会被禁用。`verso resume` 会对照固定远端检查持久化的
  tag 和 commit，再在安全时完成或重试同一次原子推送。ref 不完整或不匹配时必须手工恢复。

如果无法再精确恢复，请先检查仓库和远端。`verso abort --force` 只丢弃活动日志，刻意保留文件、
commit、tag 和远端 ref 原样。

</Callout>

即使 push 开始后本地 `HEAD` 前进，resume 仍会使用事务开始时持久化的 push URL、commit 和 tag。
...
Registry 发布、GitHub Release、二进制构建和部署仍应交给 tag 触发的 CI。
...
自动化命名发布组时，请给两条命令都添加 `--group <NAME>`。

## 原子推送细节

<details>
  <summary>查看 Verso 推送的准确 ref</summary>
